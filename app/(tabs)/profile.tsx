...
const DIET_OPTIONS = [
  { key: "balanced", label: "Balanced", icon: "nutrition-outline" as const, desc: "Equal macro distribution" },
  { key: "high-protein", label: "High Protein", icon: "barbell-outline" as const, desc: "Focus on muscle building" },
  { key: "keto", label: "Keto", icon: "flame-outline" as const, desc: "Low carb, high fat" },
  { key: "vegan", label: "Vegan", icon: "leaf-outline" as const, desc: "Plant-based nutrition" },
] as const;
...
export default function ProfileScreen() {
  ...
  function selectDiet(diet: string) {
    ...
    const presets: Record<string, Partial<typeof goals>> = {
      balanced: { dietPreference: "balanced", proteinGoal: 150, carbsGoal: 200, fatGoal: 65 },
      "high-protein": { dietPreference: "high-protein", proteinGoal: 200, carbsGoal: 150, fatGoal: 60 },
      keto: { dietPreference: "keto", proteinGoal: 120, carbsGoal: 50, fatGoal: 130 },
      vegan: { dietPreference: "vegan", proteinGoal: 100, carbsGoal: 250, fatGoal: 65 },
    };
        ...
        <GlassCard style={styles.profileHeader}>
          <LinearGradient
            colors={[
              ...
              colorScheme === "dark" ? "rgba(0,217,165,0.08)" : "rgba(0,184,148,0.04)",
            ]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
          ...
          <View style={styles.profileRow}>
            ...
            >
              <Text style={[styles.profileScore, { color: colors.text }]}>{completionScore}%</Text>
            </CalorieRing>
          </View>
        </GlassCard>
        ...
        <GlassCard>
          <View style={styles.weightRow}>
            <View style={styles.weightItem}>
              <Text style={[styles.weightLabel, { color: colors.textSecondary }]}>Current</Text>
              {editingField === "currentWeight" ? (
                <View style={styles.editRow}>
                  <TextInput
                    style={[styles.editInput, { color: colors.text, borderColor: colors.tint }]}
                    value={editValue}
                    onChangeText={setEditValue}
                ...
                <Pressable onPress={() => startEdit("currentWeight", goals.currentWeight)} style={styles.weightValueRow}>
                  <Text style={[styles.weightValue, { color: colors.text }]}>{goals.currentWeight} kg</Text>
            ...
            <View style={styles.weightItem}>
              <Text style={[styles.weightLabel, { color: colors.textSecondary }]}>Target</Text>
              {editingField === "targetWeight" ? (
                ...
                <Pressable onPress={() => startEdit("targetWeight", goals.targetWeight)} style={styles.weightValueRow}>
                  <Text style={[styles.weightValue, { color: colors.text }]}>{goals.targetWeight} kg</Text>
                  <Feather name="edit-2" size={14} color={colors.textTertiary} />
                </Pressable>
              )}
            </View>
        ...
        <View style={styles.dietGrid}>
          {DIET_OPTIONS.map((opt) => (
            <Pressable
              ...
              style={({ pressed }) => [{ opacity: pressed ? 0.8 : 1, flex: 1, minWidth: "45%" as any }]}
            >
              <GlassCard
                style={[
                  styles.dietCard,
              ...
              >
                <Ionicons
                  name={opt.icon}
                  size={22}
                  color={goals.dietPreference === opt.key ? colors.tint : colors.textSecondary}
                />
                <Text
                  style={[
                    styles.dietLabel,
                    { color: goals.dietPreference === opt.key ? colors.tint : colors.text },
                  ]}
                >
                  {opt.label}
                </Text>
                <Text style={[styles.dietDesc, { color: colors.textTertiary }]}>{opt.desc}</Text>
              </GlassCard>
            </Pressable>
          ))}
        </View>
...
function GoalRow({
  ...
  return (
    <View style={goalStyles.row}>
      <View style={[goalStyles.iconBg, { backgroundColor: accentColor + "15" }]}>
        <Ionicons name={icon as any} size={18} color={accentColor} />
      </View>
      <Text style={[goalStyles.label, { color: colors.textSecondary }]}>{label}</Text>
      {editing ? (
        <View style={goalStyles.editRow}>
          <TextInput
            style={[goalStyles.editInput, { color: colors.text, borderColor: colors.tint }]}
            value={editValue}
        ...
        <Pressable onPress={onEdit} style={goalStyles.valueRow}>
          <Text style={[goalStyles.value, { color: colors.text }]}>
            {value} <Text style={{ color: colors.textTertiary, fontSize: 13 }}>{unit}</Text>
          </Text>
...
const styles = StyleSheet.create({
  ...
  dietDesc: { fontSize: 11, fontFamily: "DMSans_400Regular", textAlign: "center" },
  recCard: {},
