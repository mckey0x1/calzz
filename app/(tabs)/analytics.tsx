...
export default function AnalyticsScreen() {
  ...
  const { weekLogs, todayLog, totalCalories, totalProtein, totalCarbs, totalFat, goals } = useNutrition();
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <GlassCard style={styles.healthScoreCard}>
          <LinearGradient
            colors={[
              colorScheme === "dark" ? "rgba(0,217,165,0.12)" : "rgba(0,184,148,0.06)",
              colorScheme === "dark" ? "rgba(139,124,247,0.12)" : "rgba(108,92,231,0.06)",
            ]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
          ...
          <View style={styles.healthScoreContent}>
            ...
            >
              <Text style={[styles.healthScoreValue, { color: colors.text }]}>{healthScore}</Text>
            </CalorieRing>
            <View style={styles.healthScoreInfo}>
              <View style={styles.healthScoreHeader}>
                <Ionicons name="sparkles" size={16} color={colors.accentEmerald} />
                <Text style={[styles.healthScoreLabel, { color: colors.accentEmerald }]}>AI Health Score</Text>
              </View>
              <Text style={[styles.healthScoreDesc, { color: colors.textSecondary }]}>
                {healthScore >= 80
        ...
        <GlassCard>
          <View style={styles.chartHeader}>
            <Text style={[styles.chartAvg, { color: colors.textSecondary }]}>
              Avg: <Text style={{ color: colors.text, fontFamily: "DMSans_700Bold" }}>{avgCalories} cal</Text>
            </Text>
            <View style={[styles.goalLine, { backgroundColor: colors.accentEmerald + "30" }]}>
              <Text style={[styles.goalLineText, { color: colors.accentEmerald }]}>Goal: {goals.dailyCalories}</Text>
            </View>
          </View>
          <WeeklyBarChart data={weeklyCalories} goal={goals.dailyCalories} colors={colors} />
        </GlassCard>
        ...
        <Text style={[styles.sectionTitle, { color: colors.text }]}>Macro Balance</Text>
        <GlassCard>
          <View style={styles.macroBalanceRow}>
            <MacroPieSlice label="Protein" value={proteinPct} color={colors.proteinColor} colors={colors} />
            <MacroPieSlice label="Carbs" value={carbsPct} color={colors.carbsColor} colors={colors} />
            <MacroPieSlice label="Fat" value={fatPct} color={colors.fatColor} colors={colors} />
          </View>
          <View style={[styles.macroBalanceBar, { backgroundColor: colors.progressRingBg }]}>
            {totalMacroGrams > 0 && (
              <>
                <View style={[styles.macroSegment, { backgroundColor: colors.proteinColor, flex: proteinPct }]} />
                <View style={[styles.macroSegment, { backgroundColor: colors.carbsColor, flex: carbsPct }]} />
                <View style={[styles.macroSegment, { backgroundColor: colors.fatColor, flex: fatPct }]} />
              </>
            )}
          </View>
        </GlassCard>
        ...
        <View style={styles.summaryRow}>
          <GlassCard style={styles.summaryCard}>
            ...
            <Text style={[styles.summaryValue, { color: colors.text }]}>
              {weeklyCalories.filter((d) => Math.abs(d.total - goals.dailyCalories) < goals.dailyCalories * 0.15).length}/7
            </Text>
            <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>Days on target</Text>
          </GlassCard>
          <GlassCard style={styles.summaryCard}>
            <Feather name="trending-up" size={20} color={colors.accentEmerald} />
            <Text style={[styles.summaryValue, { color: colors.text }]}>
              {Math.round(weeklyCalories.reduce((s, d) => s + d.total, 0) / 1000 * 10) / 10}k
            </Text>
            <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>Total cal this week</Text>
          </GlassCard>
        </View>
      </ScrollView>
    </View>
              ...
              <View style={styles.barWrapper}>
                <View
                  style={[
                    ...
                    {
                      height,
                      backgroundColor: isOverGoal ? colors.warningOrange : colors.tint,
                      borderRadius: 6,
                    },
                  ]}
                />
    ...
    <View style={styles.lineChart}>
      <Svg width={chartW} height={chartH}>
        ...
        {data.map((d, i) => {
          const x = padding + (i / (data.length - 1)) * (chartW - padding * 2);
          const y = chartH - padding - ((d.weight - minW) / (maxW - minW)) * (chartH - padding * 2);
          return <Circle key={i} cx={x} cy={y} r={4} fill={colors.accentEmerald} />;
        })}
      </Svg>
      <View style={styles.weightLabels}>
        <Text style={[styles.weightLabel, { color: colors.textTertiary }]}>{data[0]?.weight.toFixed(1)} kg</Text>
        <Text style={[styles.weightLabel, { color: colors.textTertiary }]}>{data[data.length - 1]?.weight.toFixed(1)} kg</Text>
      </View>
    </View>
  );
}
...
function MacroPieSlice({
  ...
  return (
    <View style={styles.macroSlice}>
      ...
      <Text style={[styles.macroSliceLabel, { color: colors.textSecondary }]}>{label}</Text>
      <Text style={[styles.macroSliceValue, { color: colors.text }]}>{value}%</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  healthScoreDesc: { fontSize: 13, fontFamily: "DMSans_400Regular", lineHeight: 18 },
  ...
  barChartBars: { flexDirection: "row", justifyContent: "space-between", width: "100%", height: 140, alignItems: "flex-end" },
  ...
  weightLabels: { flexDirection: "row", justifyContent: "space-between", width: "100%", marginTop: 8 },
  weightLabel: { fontSize: 12, fontFamily: "DMSans_500Medium" },
  macroBalanceRow: { flexDirection: "row", justifyContent: "space-around", marginBottom: 16 },
  ...
  macroBalanceBar: { height: 10, borderRadius: 5, flexDirection: "row", overflow: "hidden" },
  ...
  summaryLabel: { fontSize: 12, fontFamily: "DMSans_400Regular", textAlign: "center" },
});
