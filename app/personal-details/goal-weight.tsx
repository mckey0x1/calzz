...
export default function SetGoalWeightScreen() {
  ...
  const [weight, setWeight] = useState(goals.targetWeight || 120);
  ...
  const handleSave = () => {
    updateGoals({ targetWeight: weight });
    router.back();
  };

  const weightItems = Array.from({ length: 251 }, (_, i) => i + 50);
  ...
  return (
    <View style={[styles.container, { backgroundColor: "#fff" }]}>
      ...
      <View style={styles.content}>
        <Text style={styles.columnLabel}>Target Weight</Text>
        <View style={styles.pickersRow}>
          <View style={styles.weightPicker}>
            <CustomPicker items={weightItems} selectedValue={weight} onValueChange={setWeight} label="lb" />
          </View>
        </View>
      </View>
