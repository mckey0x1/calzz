...
import { View, Text, StyleSheet, Pressable, Platform, Animated, useWindowDimensions } from "react-native";
...
export default function SetGoalWeightScreen() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      flatListRef.current?.scrollToOffset({ offset: initialScrollX, animated: false });
    }, 100);
  ...
  const data = Array.from({ length: MAX_WEIGHT - MIN_WEIGHT + 1 }, (_, i) => MIN_WEIGHT + i);

  const gainText = weightState > currentWeight 
    ? "Gain Weight" 
    : (weightState < currentWeight ? "Lose Weight" : "Maintain Weight");
    ...
    <View style={[styles.container, { backgroundColor: "#fff" }]}>
      ...
      <View style={styles.content}>
        <View style={styles.titleContainer}>
          ...
          <Text style={styles.largeWeightText}>{weightState.toFixed(1)} lbs</Text>
        </View>

        <View style={styles.rulerWrapper}>
          <Animated.View style={[styles.shadedBox, { left: gainLeft, width: gainWidth }]} />
          <Animated.View style={[styles.shadedBox, { left: loseLeft, width: loseWidth }]} />
