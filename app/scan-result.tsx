...
export default function ScanResultScreen() {
  ...
  const [localData, setLocalData] = useState<any>(() => {
    if (existingEntry) {
      return {
        ...
        score: existingEntry.confidence ? existingEntry.confidence / 10 : 8,
  ...
  async function handleSave() {
    ...
    addFoodEntry({
      ...
      calories: Math.round(currentScan.calories * quantity),
      protein: Math.round(currentScan.protein * quantity),
      carbs: Math.round(currentScan.carbs * quantity),
      fat: Math.round(currentScan.fat * quantity),
      fiber: Math.round((currentScan.fiber || 0) * quantity),
      sugar: Math.round((currentScan.sugar || 0) * quantity),
      sodium: Math.round((currentScan.sodium || 0) * quantity),
      meal: mealType,
          ...
          <ScrollView
              ...
              <View style={[styles.statBox, { backgroundColor: "#F8FAFC" }]}>
                ...
                <View>
                  <Text style={styles.statBoxValue}>
                    {Math.round(currentScan.calories * quantity)}
                  </Text>
            ...
            <View style={styles.macrosGrid}>
              <MacroCard
                ...
                label="Protein"
                value={`${Math.round(currentScan.protein * quantity)}g`}
                color="#e65c5c"
              />
              <MacroCard
                icon={
                ...
                label="Carbs"
                value={`${Math.round(currentScan.carbs * quantity)}g`}
                color="#e89e5d"
              />
              <MacroCard
                icon={
                ...
                label="Fat"
                value={`${Math.round(currentScan.fat * quantity)}g`}
                color="#5a8bed"
              />
            </View>

            {/* Serving Size */}
            {/* {currentScan.servingSize && (
              <View style={styles.servingSizeBadge}>
                <Ionicons name="resize-outline" size={14} color="#6B7280" />
                <Text style={styles.servingSizeText}>
                  Serving: {currentScan.servingSize}
                </Text>
              </View>
            )} */}
            ...
            {currentScan.items && currentScan.items.length > 0 && (
              <>
                {/* <Text style={styles.sectionHeader}>Food Breakdown</Text>
                <View style={styles.breakdownContainer}>
                  {currentScan.items.map((item: any, index: number) => {
                    const methodLabels: Record<string, { label: string; icon: string; color: string }> = {
                      reference_scaling: { label: "Ref. Scaling", icon: "📐", color: "#8B5CF6" },
                      depth_estimation: { label: "Depth AI", icon: "🔍", color: "#3B82F6" },
                      segmentation: { label: "Segmented", icon: "🧩", color: "#10B981" },
                      density_conversion: { label: "Density", icon: "⚖️", color: "#F59E0B" },
                    };
                    const methodInfo = methodLabels[item.method] || { label: "Estimated", icon: "📊", color: "#6B7280" };

                    return (
                      <View key={index} style={styles.breakdownItem}>
                        <View style={styles.breakdownLeft}>
                          <View style={[styles.breakdownDot, { backgroundColor: methodInfo.color }]} />
                          <View>
                            <Text style={styles.breakdownName}>{item.name}</Text>
                            <View style={styles.breakdownMethodRow}>
                              <Text style={styles.breakdownMethodEmoji}>{methodInfo.icon}</Text>
                              <Text style={[styles.breakdownMethodText, { color: methodInfo.color }]}>
                                {methodInfo.label}
                              </Text>
                            </View>
                          </View>
                        </View>
                        <Text style={styles.breakdownGrams}>{item.estimatedGrams}g</Text>
                      </View>
                    );
                  })}
                </View> */}
